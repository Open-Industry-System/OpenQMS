...
3. **做**：在 FM 字段触发 AI（failure_mode trigger）→ 抓响应。
   - **期望**：3 required_retrievers + context_execution + generation_execution 可观测；failure_cause 上下文**不含** `work_elements` 键（DFMEA 无 4M）。
   - **断言**：同 02.4 §C。
   - **当前预期 FAIL/MISSING**：同 02.4。
   - **再做**：FE/FC/PC/DC 各触发一次 AI（共 5 个 trigger_type），重复断言。
