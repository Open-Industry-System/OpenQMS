...
**基线**: `main@eaf5793c`
**最近实现提交**: `c0b7801d fix(frontend): ignore stale route refresh results`
**当前阶段**: PR #22 已合并 `main@eaf5793c`，冲突与认证阻塞已解决，完整验证通过，待最终评审
...
- **Refresh 并发协调初版**：`authStore.tryRefreshToken()` 使用 module-scoped in-flight promise 对同一会话去重，route 与 Axios 同时刷新时仅首个 caller 提交 rotating refresh token；其余 caller 共享 promise。严格 TDD RED 为 `2 failed / 1 passed`，最小实现后 GREEN `3/3 passed`。
- **2026-09-20 认证终审阻塞修复**：新增进程内 session generation 与 refresh-token 所有权；login 开始与 logout 都会立即使旧 refresh/fetchUser 结果失效，旧成功不得覆盖新会话，旧失败不得清除新会话；跨标签 token 变化时 stale fetch 也会释放 loading。Axios 请求记录发起时 token/generation：延迟旧 401 不再发起 refresh，旧 refresh null/thrown 不再 logout/redirect 新会话；仅 `/auth/login`、`/auth/refresh`、`/auth/me` 的 403 直接 reject，`/auth/users` 等受保护管理 API 与普通业务 403 仍仅重读一次 `/auth/me`，从而关闭 refresh/me 自等待且保留角色撤销后的 UI 重验证。Axios refresh cycle 按 session owner 隔离，同一 owner 共享 cycle，不同 owner 可并发独立刷新，旧 cycle 失败不会拒绝新登录会话的请求；成功 refresh 即使旋转 refresh token 仍校验 access token，有既存 user 时主动重读 `/auth/me` 更新权限/工厂范围。严格 TDD RED：authStore 首轮 `6 failed / 3 passed`、ownership 追加 `2 failed / 9 passed`、stale login 追加 `2 failed / 11 passed`；client 首轮 `5 failed / 6 passed`、owner cycle `1 failed / 11 passed`、auth 管理 403 `1 failed / 12 passed`；route 首轮 `2 failed / 11 passed`、追加 `2 failed / 13 passed`。最终 GREEN：公开首页/auth 聚焦 `7 files / 64 tests passed`，变更文件 ESLint exit 0，production build exit 0（`6835 modules transformed`）。合并 `main@eaf5793c` 后，`make check TEST_DB=qms_test_pr22_finalreview2_20260920` 在新建库上 exit 0：backend `1975 passed / 5 skipped / 3 xfailed / 2 xpassed`，frontend `tsc --noEmit` 与 production build 均通过。
