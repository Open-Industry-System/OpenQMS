...
- **2026-09-20 认证终审阻塞修复**：新增进程内 session generation 与 refresh-token 所有权；login/logout 会使旧 refresh/fetchUser 结果失效，旧成功不得覆盖新会话，旧失败不得清除新会话，不同 owner 的 refresh 不共享 promise。Axios 请求记录发起时 token/generation：延迟旧 401 不再发起 refresh，旧 refresh null/thrown 不再 logout/redirect 新会话；`/auth/*` 403 直接 reject，业务 403 仍仅重读一次 `/auth/me`，从而关闭 `/auth/refresh` 403 ↔ `/auth/me` 401 自等待。严格 TDD RED：authStore `6 failed / 3 passed`、client `5 failed / 6 passed`、route `2 failed / 11 passed`；GREEN：合并后公开首页/auth 聚焦 `7 files / 56 tests passed`，变更文件 ESLint exit 0，production build exit 0（`6835 modules transformed`）。合并 `main@eaf5793c` 后，`make check TEST_DB=qms_test_pr22_merge_20260920` 在新建库上 exit 0：backend `1975 passed / 5 skipped / 3 xfailed / 2 xpassed`，frontend `tsc --noEmit` 与 production build 均通过。
