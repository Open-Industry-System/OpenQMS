...
**最近提交**: 公开首页最终 refresh 协调修复已完成：store 级共享 in-flight promise 消除 route/Axios 并发刷新竞态
...
- **Refresh 并发协调最终跟进**：`authStore.tryRefreshToken()` 新增单一 module-scoped in-flight promise，route 与 Axios 同时刷新时仅首个 caller 读取并提交 rotating refresh token；其余 caller 共享同一 promise。成功仍只写入返回的 token pair 并更新 Zustand token，失败仍 fail-closed logout；`finally` 清除 promise 后允许后续独立刷新；无 refresh token 仍直接返回 `null`。严格 TDD RED 为 `authStore.test.ts` `2 failed / 1 passed`，两项失败均为 `apiRefreshToken` 期望 1 次、实际 2 次；最小实现后 GREEN `3/3 passed`。最终 focused `3 files / 19 tests passed`、公开首页 aggregate `7 files / 42 tests passed`、变更文件 ESLint exit 0、production build exit 0（`6835 modules transformed`）。`make check TEST_DB=qms_test_public_home_refresh_coord` 明确新建并迁移未使用数据库后 exit 0：backend `1957 passed / 5 skipped / 3 xfailed / 2 xpassed`（25 warnings），frontend `tsc --noEmit` 与 production build 均通过；`App.tsx`、`client.ts` 及其既有 route/interceptor 行为均未修改。
