...
describe("deleteSubtree — effect survivor handling", () => {
  ...
  const edge = (source: string, target: string, type: string): GraphEdge => ({ source, target, type });
  ...
  it("keeps an effect shared with a surviving function, deletes a private effect", () => {
    ...
    const edges = [
      edge("fn1", "fm1", "HAS_FAILURE_MODE"),
      edge("fn2", "fm2", "HAS_FAILURE_MODE"),
      edge("fm1", "fe1", "EFFECT_OF"),
      edge("fm1", "fe2", "EFFECT_OF"),
      edge("fm2", "fe1", "EFFECT_OF"),
    ];
    ...
    expect(result.edges).toEqual([edge("fn2", "fm2", "HAS_FAILURE_MODE"), edge("fm2", "fe1", "EFFECT_OF")]);
  });
});
