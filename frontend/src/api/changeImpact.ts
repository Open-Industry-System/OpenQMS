...
export interface PaginatedChangeImpactResponse {
  items: ChangeImpactAnalysis[];
  total: number;
  page: number;
  page_size: number;
}

export async function listChangeImpacts(fmeaId: string): Promise<PaginatedChangeImpactResponse> {
  const resp = await client.get(`/change-impact/fmea/${fmeaId}`);
  return resp.data;
}

export async function listAllChangeImpacts(productLineCode?: string): Promise<PaginatedChangeImpactResponse> {
