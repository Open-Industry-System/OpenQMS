...
触发 `failure_mode`/`failure_effect`/`failure_cause`/`prevention_control`/`detection_control` 推荐时，后端必须查询 4 来源，通过 `source_executions[]` 可观测（同 02.4）。
...
| 通过条件 | 失效链边方向正确 + FM 挂 DFMEA 功能节点 + AI 查全 4 来源（source_executions 可观测）+ 采纳留痕 + 审计 |
| 失败条件（FAILED） | 失效链边方向反了；FM 挂错层级；AI 未查 #2/#3（source_executions 缺）；未审计 |
