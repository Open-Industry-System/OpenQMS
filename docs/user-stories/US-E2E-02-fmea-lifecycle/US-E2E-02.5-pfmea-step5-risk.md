...
**关联 skill**: `verify-fmea-lifecycle-pfmea-step5-risk`
...
**作为** 前期策划质量工程师，**我想** 在向导 Step5 评分：三段式严重度 S（本厂 severity_plant / 客户 severity_customer / 终端用户 severity_user，取最大值）+ 频度 O（FailureCause.occurrence）+ 探测度 D（DetectionControl.detection），自动计算 AP（H/M/L），并维护 CC/SC 特殊特性，
...
2. 系统自动计算 AP（`utils/fmea.ts calculateAP`，AIAG-VDA AP 表共享）。
...
- AP 自动计算正确（S×O×D → H/M/L，查 `utils/fmea.ts` AP 表）。
...
| 关键字段 | FE.severity_plant/customer/user；FC.occurrence；DC.detection；AP |
| 边类型 | 无新增（更新现有节点） |
...
| 通过条件 | 三段式 S 均>0 + AP 计算正确 + CC/SC 写 classification + AI 查全 4 来源 + 采纳留痕 + 审计 |
| 失败条件（FAILED） | 三段式 S 任一为 0；AP 计算错误；CC/SC 写错字段；AI 未查 #2/#3；未审计 |
