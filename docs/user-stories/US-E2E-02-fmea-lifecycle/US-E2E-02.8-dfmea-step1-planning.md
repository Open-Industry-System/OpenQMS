...
**关联 skill**: `verify-fmea-lifecycle-dfmea-step1-planning`
**前置**: 无（向导第一步）
...
### 结构完整性
- `wizardScope` 元数据含 5T 字段。

### AI 推荐知识库查询契约（AI_REQUIRED=true）
触发 `dfmea_tool`/`dfmea_trend` 推荐时，后端必须查询 4 来源（同 02.1，但查询 DFMEA 节点）。
...
| 失败条件（FAILED） | 字段缺失；AI 未查 #2/#3；推荐无 source；未审计 |
