...
> **v4 变更（2026-07-09）**：LLM 校正后的内容**不得自动标记 `passed`**——结构校验只能保证页/section/label 不变，无法保证 LLM 未篡改事实值（状态/验证结果/关闭结论）。故：首轮即通过（无校正，内容 DB-faithful）→ `passed`；任何经 LLM 校正采用的内容 → 强制 `needs_review` + 报告标注「需人工复核确认未编造数据」。守住 §70「内容来自落库数据」与 §101「不编造数据」。
...
- **sub-agent 审查**（LLM 已配置时）：生成后自动审查，不通过则校正重生成，3 轮上限；**首轮即通过（无校正，内容 DB-faithful）→ `passed`；任何经 LLM 校正采用的内容 → `needs_review`（强制人工复核，不自动 passed，守 §70/§101）；达上限仍不合格 → `needs_review`**（返回最后 PPT + 审查报告，不报错）。
