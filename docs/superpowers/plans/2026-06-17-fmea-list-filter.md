...
Create `backend/tests/test_fmea_list_filter.py`:
...
def _make_doc(document_no: str, title: str, fmea_type: str = "PFMEA",
              graph_data: dict | None = None, factory_id=uuid.UUID("00000000-0000-0000-0000-000000000001")):
    return FMEADocument(
        fmea_id=uuid.uuid4(),
...
async def test_fmea_type_filter(db, default_factory):
    pfmea = _make_doc("PFMEA-2026-001", "过程 FMEA", "PFMEA", factory_id=default_factory.id)
    dfmea = _make_doc("DFMEA-2026-001", "设计 FMEA", "DFMEA", factory_id=default_factory.id)
    db.add_all([pfmea, dfmea])
    await db.flush()

    items, total = await list_fmeas(db, 1, 20, factory_id=default_factory.id, fmea_type="PFMEA")
    assert total == 1
    assert items[0].document_no == "PFMEA-2026-001"
```
...
async def test_search_by_document_no_case_insensitive(db, default_factory):
    a = _make_doc("PFMEA-2026-001", "Alpha", factory_id=default_factory.id)
    b = _make_doc("DFMEA-2026-001", "Beta", factory_id=default_factory.id)
    db.add_all([a, b])
    await db.flush()

    items, total = await list_fmeas(db, 1, 20, factory_id=default_factory.id, search="pfmea")
    assert total == 1
    assert items[0].document_no == "PFMEA-2026-001"
```
...
async def test_search_by_title(db, default_factory):
    a = _make_doc("PFMEA-2026-001", "焊接工艺失效分析", factory_id=default_factory.id)
    b = _make_doc("DFMEA-2026-001", "Other", factory_id=default_factory.id)
    db.add_all([a, b])
    await db.flush()

    items, total = await list_fmeas(db, 1, 20, factory_id=default_factory.id, search="焊接")
    assert total == 1
...
async def test_search_blank_skips_filter(db, default_factory):
    ...
    _, total = await list_fmeas(db, 1, 20, factory_id=default_factory.id, search="   ")
    assert total == 2
```
...
async def test_search_escapes_sql_wildcards(db, default_factory):
    a = _make_doc("PFMEA-2026-001", "Alpha", factory_id=default_factory.id)
    b = _make_doc("PFMEA-2026-002", "100%_yield", factory_id=default_factory.id)
    db.add_all([a, b])
    await db.flush()

    # "%" and "_" are escaped → literal match, only b's title contains "%"
    items, total = await list_fmeas(db, 1, 20, factory_id=default_factory.id, search="%")
    assert total == 1
...
async def test_high_rpn_with_fmea_type_filters_first(db, default_factory):
    high_pfmea = _make_doc("PFMEA-2026-H1", "High PFMEA", "PFMEA", _high_rpn_graph(), default_factory.id)
    high_dfmea = _make_doc("DFMEA-2026-H1", "High DFMEA", "DFMEA", _high_rpn_graph(), default_factory.id)
    db.add_all([high_pfmea, high_dfmea])
    await db.flush()

    items, total = await list_fmeas(db, 1, 20, factory_id=default_factory.id, high_rpn=True, fmea_type="PFMEA")
    assert total == 1
    assert items[0].fmea_type == "PFMEA"
    assert items[0].document_no == "PFMEA-2026-H1"
```
...
async def test_high_rpn_with_search_filters_first(db, default_factory):
    h_a = _make_doc("PFMEA-2026-H1", "焊接失效", "PFMEA", _high_rpn_graph(), default_factory.id)
    h_b = _make_doc("PFMEA-2026-H2", "Other", "PFMEA", _high_rpn_graph(), default_factory.id)
    db.add_all([h_a, h_b])
    await db.flush()

    items, total = await list_fmeas(db, 1, 20, factory_id=default_factory.id, high_rpn=True, search="焊接")
    assert total == 1
...
```typescript
  const fetchData = (p: number = page) => {
    ...
    listFMEAs({
      ...
      fmea_type: (filterType as "PFMEA" | "DFMEA") || undefined,
...
参考已有 `*.test.tsx`（如 `frontend/src/pages/capa/CAPADetailPage.test.tsx`）的 Testing Library + MemoryRouter 用法。需 mock `listFMEAs`/`createFMEA` 与全局 store。

- [ ] **Step 1: 检查现有测试的 mock 套路**

Run: `sed -n '1,60p' frontend/src/pages/capa/CAPADetailPage.test.tsx` 与 `sed -n '1,50p' frontend/src/pages/dashboard/dashboardLayoutUtils.test.ts`，确认 vitest mock、MemoryRouter、i18n 初始化的既有写法，新测试沿用同款 setup。
...
const listFMEAsMock = vi.fn();
vi.mock("../../../api/fmea", () => ({
  listFMEAs: (...args: unknown[]) => listFMEAsMock(...args),
  createFMEA: vi.fn(),
}));

vi.mock("../../../store/authStore", () => ({
  useAuthStore: () => ({ user: { user_id: "u1", role: "admin" } }),
}));
...
vi.mock("../../../store/productLineStore", () => ({
  useProductLineStore: () => ({ selected: "DC-DC-100" }),
}));
...
beforeEach(() => {
  listFMEAsMock.mockResolvedValue({ items: [], total: 0, page: 1, page_size: 20 });
});

describe("FMEAListPage filters", () => {
  it("renders without crashing and requests first page", async () => {
    renderAt("/fmea");
    expect(listFMEAsMock).toHaveBeenCalled();
  });
});
```
...
```typescript
  it("reads legacy ?risk=high and sends high_rpn=true", async () => {
    ...
    await vi.waitFor(() => {
      const call = listFMEAsMock.mock.calls[listFMEAsMock.mock.calls.length - 1][0];
...
```typescript
  it("changing type filter sends fmea_type and resets to page 1", async () => {
    renderAt("/fmea");
    await vi.waitFor(() => expect(listFMEAsMock).toHaveBeenCalled());
...
**3. Type consistency:** service 签名 `fmea_type`/`search` keyword params（Task 1）↔ API 透传（Task 2）↔ client `fmea_type?: "PFMEA"|"DFMEA"; search?: string`（Task 3）↔ 页面调用 `(filterType as "PFMEA"|"DFMEA")`（Task 5）一致。`updateFilters`/`onStatusChange`/`onTypeChange`/`onHighRpnChange`/`onSearch`/`onReset` 命名在 Task 5 内自洽，Task 6 测试不直接调用这些函数（通过 UI 交互），无跨任务名冲突。
