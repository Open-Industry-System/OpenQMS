...
- [ ] **Step 0: 确认当前 alembic head（并行安全）**

Run: `cd backend && alembic heads`
Expected: 当前真实 head（本仓库多分支：tenant+platform 在 `20260708_d7_node_action_status` 合并；`grep -c "(head)"` 应为 1）。**记录输出的 revision id**，填入 Step 5 迁移文件的 `down_revision`。若 `alembic heads` 输出多于 1 个 head，本迁移接 **tenant 分支的当前 head**（`20260708_d7_node_action_status` 带有 `(tenant, platform)` 标签即 tenant 分支 head）；若 01.3 状态机切片已新增迁移，接到那个新 head。**不要硬写 `20260708_d7_node_action_status`——以 `alembic heads` 实时输出为准。**
...
# ↓ 从 Step 0 的 `alembic heads` 输出填入当前真实 head（不要硬写）
down_revision = "<填入 alembic heads 的当前 head，如 20260708_d7_node_action_status>"
branch_labels = None
depends_on = None
...
- [ ] **Step 6: Add frontend visibility tests**

`frontend/src/pages/capa/__tests__/CAPADetailPage.ppt.test.tsx`（vitest + @testing-library/react，照现有页面测试模式）:
```tsx
import { render, screen } from "@testing-library/react";
import { usePermission } from "../../../hooks/usePermission";

// 测试：生成PPT 按钮可见性 = canCreate("capa")（L2）+ D8_CLOSURE/ARCHIVED

jest.mock("../../../hooks/usePermission");
const mockedUsePermission = usePermission as jest.MockedFunction<typeof usePermission>;

function renderWith(status: string, capaLevel: number) {
  mockedUsePermission.mockReturnValue({
    canCreate: (m: string) => m === "capa" && capaLevel >= 2,
    canEdit: (m: string) => m === "capa" && capaLevel >= 3,
    canApprove: () => false, canView: () => true, canAdmin: () => false, getLevel: () => capaLevel as any,
  } as any);
  // render CAPADetailPage with capa={status, ...} --照页面现有测试 fixture
  // ... (省略页面 mount 细节，照 CAPADetailPage 现有 vitest fixture)
}

test("engineer (capa=2) sees 生成PPT button on D8_CLOSURE", () => {
  renderWith("D8_CLOSURE", 2);
  expect(screen.getByText(/生成 PPT|Generate PPT/)).toBeInTheDocument();
});

test("viewer (capa=1) does NOT see 生成PPT button on D8_CLOSURE", () => {
  renderWith("D8_CLOSURE", 1);
  expect(screen.queryByText(/生成 PPT|Generate PPT/)).not.toBeInTheDocument();
});

test("engineer (capa=2) does NOT see button on D7_PREVENTION (not closed)", () => {
  renderWith("D7_PREVENTION", 2);
  expect(screen.queryByText(/生成 PPT|Generate PPT/)).not.toBeInTheDocument();
});
```
> 若 CAPADetailPage 无现有 vitest fixture，则用 `canCreate` 纯函数测试代替：断言 `canCreate` 逻辑（capa=2 → true，capa=1 → false）+ 按钮渲染条件表达式覆盖。关键是验证 L2（非 L3）即可见。

- [ ] **Step 7: Verify build + tests**

Run: `cd frontend && npm run build && npm test -- --run src/pages/capa/__tests__/CAPADetailPage.ppt.test.tsx`
Expected: PASS (tsc -b + vite build + vitest 3 passed)

- [ ] **Step 8: Commit**

```bash
cd /Users/sam/Documents/Code/OpenQMS
git add frontend/src/api/capa.ts frontend/src/pages/capa/CAPADetailPage.tsx frontend/src/pages/capa/__tests__/CAPADetailPage.ppt.test.tsx frontend/src/pages/admin/ReviewSkillsPage.tsx frontend/src/locales frontend/src/types/index.ts frontend/src/App.tsx
