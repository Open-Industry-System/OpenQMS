...
- 迁移号 `20260708_capa_ppt_and_review_skill`。`down_revision` **不写死**——执行 Task 1 Step 0 跑 `alembic heads` 取当前真实 head 填入（本仓库是多分支 alembic，tenant+platform 在 `20260708_d7_node_action_status` 合并；01.3 状态机切片进行中可能新增迁移，故必须实时确认，不可硬写）
...
- 生成权限 engineer+（`PermissionLevel.CREATE` = L2；seed `quality_engineer` capa=2，见 `app/seed.py:285`），状态门控 `D8_CLOSURE`/`ARCHIVED`。**注意：非 `PermissionLevel.EDIT`（L3=manager）**
...
- Modify: `backend/app/models/__init__.py` (import AgentReviewSkill + add CapaPptExport to capa tuple — 元数据注册 + autogenerate 一致性)
- Test: `backend/tests/test_migration_ppt_and_skill.py`

**Interfaces:**
- Consumes: `Base` from `app.database`（`Base = TenantBase`，`app/models/base.py` 不存在）
- Produces: `CapaPptExport` model (table `capa_ppt_export`), `AgentReviewSkill` model (table `agent_review_skill`), migration revision `20260708_capa_ppt_and_review_skill` chaining 当前 alembic head

- [ ] **Step 0: 记录当前 alembic head（并行安全）**

Run: `cd backend && alembic heads`
Expected: 单 head（`grep -c "(head)"` 应为 1）。本仓库多分支：tenant+platform 在 `20260708_d7_node_action_status` 合并（输出形如 `20260708_d7_node_action_status (tenant, platform) (head)`）。若输出多于 1 个 head，取带 `(tenant, ...)` 标签的那个；若 01.3 状态机切片已新增迁移，取那个新 head。

**把输出的 revision id 字符串记下来**（例 `20260708_d7_node_action_status`）——Step 5 创建迁移文件时必须把它原样写入 `down_revision = "<Step 0 记录的 head>"`。**保留占位符 `<...>` 不替换 = 无效迁移，`alembic upgrade` 会立即报错 `Can't locate revision`。**
...
> `Base` 来自 `app.database`（`Base = TenantBase`），与 `app/models/capa.py:8` 一致（已验证）。
...
- [ ] **Step 4b: Register models in `__init__.py`**

`backend/app/models/__init__.py` 注册所有模型供 PlatformBase/TenantBase 元数据收集 + autogenerate 一致性。两处改动：

1. 现有 capa import tuple（约第 19 行 `from app.models.capa import (...)`）加 `CapaPptExport`：
```python
...
2. 新增 AgentReviewSkill 导入（按字母序插入，紧邻 `from app.models.agent import (...)` 之后）：
```python
...
`backend/alembic/versions/20260708_capa_ppt_and_review_skill.py`。**⚠️ 创建后立即把 `down_revision` 的 `REPLACE_WITH_HEAD_FROM_STEP_0` 替换为 Step 0 记录的真实 head 字符串；同时把 docstring 的 `Revises:` 行同步替换。不替换 = 无效迁移。**

```python
"""Add capa_ppt_export + agent_review_skill tables (US-E2E-01.10)

Revision ID: 20260708_capa_ppt_and_review_skill
Revises: REPLACE_WITH_HEAD_FROM_STEP_0
...
down_revision = "REPLACE_WITH_HEAD_FROM_STEP_0"
branch_labels = None
depends_on = None
...
git add backend/alembic/versions/20260708_capa_ppt_and_review_skill.py backend/app/models/capa.py backend/app/models/agent_review_skill.py backend/app/models/__init__.py backend/tests/test_migration_ppt_and_skill.py
...
from datetime import UTC, datetime
from app.core.tenant import tenant_schema
from app.models.audit import AuditLog              # db.add(AuditLog(...)) 写 PPT_GENERATED 审计
...
// usePermission 解构出 canCreate（L2=CREATE）；不能用 canEdit（L3=EDIT，engineer L2 看不到入口）
...
// 在推进按钮区附近：engineer+（canCreate=L2）可见；D8_CLOSURE/ARCHIVED
...
export default function ReviewSkillsPage() {
  ...
  const load = async () => {
    const r = await client.get("/admin/review-skills");  // axios baseURL="/api" → /api/admin/review-skills
    setSkills(r.data);
  };
  useEffect(() => { load(); }, []);
...
- [ ] **Step 6: Extend existing `CAPADetailPage.test.tsx` with PPT button visibility tests**

`CAPADetailPage.test.tsx` 已存在（`frontend/src/pages/capa/CAPADetailPage.test.tsx`）且已 mock `../../api/capa`、`useAuthStore`、`renderPage()` helper。**扩展现有文件**，在末尾追加一个 `describe` 块——复用其 `mockCapa`/`renderPage`/`useAuthStore.setState({ permissions })` 模式（权限经 store → `usePermission` 读 `permissions[module]`，**不要** 直接 mock `usePermission`）。

文件顶部 import 区无需改动（`render/screen/waitFor`、`vi`、`capaApi` 已 import）。末尾追加：
```tsx
describe("CAPADetailPage 生成PPT button visibility", () => {
  ...
  it("viewer (capa=1 VIEW) does NOT see 生成PPT button on D8_CLOSURE", async () => {
    ...
    renderPage();
    await waitFor(() => {
      expect(screen.getByText(/8D Report|关闭结论|Closure/)).toBeInTheDocument();
    });
...
> **为什么用 `useAuthStore.setState({ permissions: { capa: N } })` 而非 mock `usePermission`**：现有测试 + `usePermission` 实现都从 `useAuthStore` 的 `permissions[module]` 读等级（`usePermission.ts:19-20`）。按真实数据流走，避免 mock 漂移。`canCreate` = `getLevel("capa") >= PermissionLevel.CREATE`（CREATE=2）：capa=2 → true，capa=1 → false。
> **按钮文案**：现有测试用 en-US 文案（"AI Draft"），故 PPT 按钮断言用 `/Generate PPT|生成 PPT/` 双匹配兜底。
> **`generatePpt` 无需单独 mock**：`vi.mock("../../api/capa")` 已 auto-mock 全部导出为 `vi.fn()`；可见性测试不触发点击，无需配置返回值。

- [ ] **Step 7: Verify build + tests**

Run: `cd frontend && npm run build && npm test -- --run src/pages/capa/CAPADetailPage.test.tsx`
Expected: PASS (tsc -b + vite build; vitest 该文件 8 passed = 原有 5 + 新增 PPT 可见性 3)

- [ ] **Step 8: Commit**

```bash
cd /Users/sam/Documents/Code/OpenQMS
git add frontend/src/api/capa.ts frontend/src/pages/capa/CAPADetailPage.tsx frontend/src/pages/capa/CAPADetailPage.test.tsx frontend/src/pages/admin/ReviewSkillsPage.tsx frontend/src/locales frontend/src/types/index.ts frontend/src/App.tsx
git commit -m "feat(01.10): frontend generatePpt + review report + admin ReviewSkillsPage (canCreate L2 gate)"
```
...
- `PermissionLevel.CREATE`（L2）— Task 6 代码 / Global Constraint / test seed capa=2 ✅ 一致（第 3 轮修复）
- `Base` from `app.database`（非 `app.models.base`）— Task 1 ✅（第 3 轮修复）
- 前端 `client.get("/admin/review-skills")`（baseURL `/api`）— Task 7 ✅（第 3 轮修复）
- `SupplierRiskAlert` from `app.models.supplier_risk`（非 `supplier`）— Task 4 ✅（第 3 轮修复）

## 第 3 轮修复（adversarial review，2026-07-08）

复审 7 项 + 2 项草稿级，均已对照实际代码验证后修正：

1. **权限级别不一致（阻断）**：`PermissionLevel.EDIT`（L3=manager）→ `PermissionLevel.CREATE`（L2=engineer，seed `quality_engineer` capa=2，`app/seed.py:285`）。代码 / Global Constraint / test seed 全部统一到 CREATE。新增 viewer（capa=1）403 测试 + `_seed_perm` 改 upsert（可降级测 viewer）。
2. **前端 /api/api/admin 双前缀（阻断）**：axios `baseURL="/api"`，`client.get("/api/admin/...")` → `/api/api/admin/...`。改为 `client.get("/admin/review-skills")`。backend 测试用 ASGITransport 直连全路径 `/api/admin/...` 不变。
3. **AgentReviewSkill Base 导入路径不存在（阻断）**：`app/models/base.py` 不存在；`Base = TenantBase` 来自 `app.database`（与 `app/models/capa.py:8` 一致）。改 `from app.database import Base`。
4. **固定单 skill 未执行（重要）**：PUT/GET 任意 name 会创建新 skill。加 `if name != "capa_ppt_review": raise 404` + 非默认 name 404 测试。
5. **SupplierRiskAlert 导入路径错（重要）**：模型在 `app/models/supplier_risk.py:12`（非 `supplier.py`）。原 `try/except ImportError` 会静默丢数据。改直接 import + 新增 risk alert 联动附录测试（含 Supplier FK 种子，NOT NULL 字段对齐：short_name/created_by）。
6. **list_skills 租户自定义后返回两条同名（重要）**：改按 name 去重，tenant 覆盖 public。新增去重测试。
7. **alembic down_revision 并行安全（重要）**：Task 1 加 Step 0 `alembic heads` 确认（当前 head `20260708_d7_node_action_status`，01.3 在进行中可能新增迁移，接到真实 head）。

草稿级：
- 删除 `Presentation.__call__.__self__` 占位行（Task 4 测试）。
- 迁移测试 `inspect(db.sync_engine)` → `to_regclass('表名')`（不依赖 sync_engine，async fixture 安全）。

## 第 4 轮修复（adversarial review，2026-07-08）

复审 4 项，均已对照实际代码验证后修正：

1. **前端按钮仍用 `canEdit`（L3）（阻断）**：`canEdit` = `>= EDIT`（L3，`usePermission.ts:25`），engineer（L2）看不到入口。改用 `canCreate`（L2，`usePermission.ts:24`）+ `const { canCreate } = usePermission()`。新增前端 vitest：capa=2 按钮可见 / capa=1 不可见 / D7_PREVENTION 不可见。
2. **alembic head 写死为假设（阻断）**：Global Constraint + 迁移代码 `down_revision` 不再硬写 `20260708_d7_node_action_status`。改为 Step 0 跑 `alembic heads` 取当前真实 head 填入（本仓库多分支：tenant+platform 在该 revision 合并；01.3 进行中可能新增迁移）。迁移 `down_revision` 用占位符 `<填入 alembic heads 的当前 head>`。
3. **新模型未注册到 `models/__init__.py`（重要）**：Task 1 加 Step 4b——`AgentReviewSkill` import + `CapaPptExport` 加入 capa tuple（元数据注册 + autogenerate 一致性）。Files 列表 + commit 加 `__init__.py`。
4. **测试数量期望不一致（重要）**：Task 2 `all 5`→`all 6`、Task 3 `all 4`→`all 5`、Task 4 `all 5`→`all 6`、Task 6 `all 4`→`all 5`（Task 5 `all 5` 已正确）。

## 第 5 轮修复（adversarial review，2026-07-08）

复审 3 项，均已对照实际代码验证后修正：

1. **Task 6 用 `AuditLog` 未 import（阻断）**：`api/capa.py` 现未 import `AuditLog`，新增 `from app.models.audit import AuditLog`（路径与 `models/__init__.py` 一致）。否则 `db.add(AuditLog(...))` NameError。
2. **前端测试是占位代码 + 用 jest 而非 vitest（重要）**：改为**扩展现有 `CAPADetailPage.test.tsx`**（已存在，已 `vi.mock("../../api/capa")` + `useAuthStore` + `renderPage()`），追加 PPT 可见性 `describe` 块。用 `vi`/`vi.mocked`（非 `jest`）+ `useAuthStore.setState({ permissions: { capa: N } })`（按真实数据流，非 mock `usePermission`）。3 个测试：capa=2 D8_CLOSURE 可见 / capa=1 不可见 / D7_PREVENTION 不可见。删除"省略 mount 细节"占位符。Step 8 commit `git add` 改为现有测试文件路径。
3. **alembic `down_revision` 占位符会被误复制（中等）**：占位符改 `REPLACE_WITH_HEAD_FROM_STEP_0`（醒目，非尖括号），Step 0 改为"记录 head"，Step 5 加⚠️提示"创建后立即替换为 Step 0 记录值，docstring `Revises:` 同步替换，不替换 = 无效迁移 `Can't locate revision`"。

## 第 6 轮修复（adversarial review，2026-07-08）

复审 2 项，均已修正：

1. **`_validate_ppt_content` 未真正校验 D2-D8 内容非空（重要）**：原代码只查 `sections` 列表非空，但 `generate_content` 用 `capa.d3_interim or ""` 生成 `{"label":"措施","value":""}`——空字符串会通过。改为查 `all(s.value.strip() empty)`。新增 `test_validate_ppt_content_empty_d3_value`（d3_interim="" → 断言返回 D3 issue）。Task 4 测试数 6→7。
2. **`skill is None` 未显式处理（次要）**：`get_by_name` 返回 None 时，`_subagent_review` 访问 `skill.content` → AttributeError 被 `except Exception` 误归"LLM 调用异常"，掩盖配置/seed 问题。改为 `skill is None` 直接返回 `ReviewResult("needs_review", 0, {"issues": ["审查 skill 未配置..."]})`。新增 `test_skill_not_configured_reports_config_issue`（断言不调 complete_json + report.issues 含 "skill"）。Task 5 测试数 5→6，import 加 `agent_review_skill_service`。

## 第 7 轮修复（adversarial review，2026-07-08）

复审 1 项，已修正：

1. **内置校验漏掉 D1 团队页（重要）**：seed 审查标准（迁移 `DEFAULT_REVIEW_SKILL_CONTENT`）写 "D1-D8 各页非空"，但 `_validate_ppt_content` 只查 D2-D8。D1 内容来自 `capa.d1_team or []`，团队为空时 D1 页 sections=[] 但当前规则不报 issue（LLM 未配置 skipped 时直接进入导出）。改为校验列表加 `(1, "D1")`，与 seed 标准 + 8D 语义（关闭报告须有团队）一致。新增 `test_validate_ppt_content_empty_d1_team`（d1_team=[] → 断言 D1 issue）。Task 4 测试数 7→8。