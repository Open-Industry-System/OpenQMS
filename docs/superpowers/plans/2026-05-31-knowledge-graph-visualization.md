...
```typescript
import { useRef } from "react";  // 已有 useState/useEffect/useCallback/useMemo，追加 useRef
import { Dropdown, Menu } from "antd";
import { GraphCanvas, GraphToolbar, NodeDetailDrawer, GraphLegend } from "../../../components/graph";
import type { GraphLayout } from "../../../components/graph";
import type { GraphNode as APIGraphNode } from "../../../api/graph";
import { getImpactChain, getCauseChain } from "../../../api/graph";
```
...
const loadGraphData = useCallback(async () => {
  ...
  try {
    ...
    const nodes = (doc.graph_data?.nodes || []).map((n) => ({
      id: n.id,
      label: n.type,
      properties: { ...n },
    }));
    const edges = (doc.graph_data?.edges || []).map((e) => ({
      source: e.source,
      target: e.target,
      label: e.type,
    }));
      ...
      useEffect(() => {
        ...
        if (tabParam === "graph") {
          ...
          if (highlightParam) {
            // 等图谱加载完成后高亮目标节点
            setTimeout(() => {
              setHighlightNodes([highlightParam]);
              setDimOthers(true);
            }, 500);
          }
        }
      ...
      {graphLoading ? (
        ...
        <GraphCanvas
          ...
          onNodeContextMenu={(node, evt) => {
            // Ant Design Dropdown 右键菜单
            const menu = (
              <Menu
                onClick={({ key }) => {
                  if (key === "impact") handleTraceImpact(node.id);
                  if (key === "cause") handleTraceCause(node.id);
                }}
              >
                <Menu.Item key="impact">追溯影响</Menu.Item>
                <Menu.Item key="cause">追溯原因</Menu.Item>
              </Menu>
            );
            // Dropdown 通过 open/coordinates 控制，此处记录坐标供 Dropdown 组件使用
            // 实际实现：用一个 state 控制右键菜单的显示位置和内容
          }}
        />
      ) : (
...
**注意:** 右键菜单使用 Ant Design Dropdown 组件实现，保持 React 状态树纯净和样式统一。G6 的 `node:contextmenu` 事件提供坐标，Dropdown 通过受控 `open` + `overlay` 渲染菜单项。
              ...
              <Row gutter={16}>
                ...
                <Col span={6}>
                  <Card>
                    <Statistic
                      title="高风险失效模式"
                      value={stats.high_risk_failure_modes?.length || 0}
                ...
                <Col span={6}>
                  <Card>
                    <Statistic
                      title="节点类型数"
                      value={Object.keys(stats.node_type_distribution || {}).length}
                    />
                  </Card>
                </Col>
              </Row>
              <Card title="高风险失效模式 Top 10" extra={<Tag color="red">RPN ≥ 100</Tag>}>
                <Table
                  dataSource={stats.high_risk_failure_modes || []}
                  columns={riskColumns}
...
- `GraphNode` / `GraphEdge` 类型在 `api/graph.ts` 中定义，与 spec 中 4.1 数据契约一致
