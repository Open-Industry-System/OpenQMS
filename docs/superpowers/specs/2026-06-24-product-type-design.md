...
- `global` → `None`（不过滤，沿用现有语义；下游已由 `user_product_lines` / 工厂 scope 收敛）。
...
- 8D 召回仍按 `user_product_lines` 过滤（同类型跨产品线历史通过 `user_product_lines` 内的同类产品线自然包含，无需额外改动）。
...
- **QA 同步**：`ask()` 复用 `semantic_search(product_line_code=...)`（`search_service.py:185`），`QARequest` 已有 `product_line_code`（`schemas/search.py:42`）。本次给 `semantic_search` 与 `ask` 同时增加 `product_type_code` 参数，`SearchRequest` 与 `QARequest` schema 同步加 `product_type_code: str | None`，前端 `SemanticSearchTab` 在 search 与 QA 两种模式下都传 `product_type_code`，避免 QA 模式下类型过滤不一致。
...
- 改：`types/index.ts`、`api/recommendation.ts`（scope Literal 三档）、`api/productLine.ts`、`api/search.ts`（SearchRequest + QARequest 加 product_type_code）、`pages/graph/SemanticSearchTab.tsx`、推荐触发组件（`SmartSuggestionDropdown`/`InlineRecommendations`）、`App.tsx`（路由）、`components/layout/AppLayout.tsx`（菜单）、i18n 资源。
