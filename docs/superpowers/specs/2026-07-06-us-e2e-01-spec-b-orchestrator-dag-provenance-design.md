...
6. **6 类新源遵循现有 Source 接口 + 强制 async `should_skip` + factory_id 隔离（R3+R6+R12-修复合约一致性）**：`async retrieve(context) -> list[RecommendationCandidate]`，构造器 `(db, embedding_provider=None)`，`name` 类属性 = 内部 source 标识。`match_source` 外部值按下表与内部 `source` 映射（`to_d4_schema` 已有 `rule_engine→rule` 映射模式，新源沿用）。**6 类新源 MUST 实现 `async def should_skip(context) -> str | None`**（R6-修复：async，因需 `await db.execute(...)` 查底层数据存在性，如 SPC 查 `spc_alarms` count、IQC 查 `iqc_inspections` 不良 count）：返回 reason → 编排器标 `skipped`（无数据）；返回 None → `retrieve`，`[]` 即 `done(0)`（有数据 0 命中），非空即 `done(N)`。编排器 `await should_skip(context)`（同步调用会返回 coroutine 被误判 truthy → 误 skipped）。**factory_id 隔离（R12-修复）**：`RecommendationContext` 加 `factory_id: uuid.UUID | None`（API handler 构造时传 `capa.factory_id`）；6 类新源 `should_skip`/`retrieve` 查询必须 `WHERE factory_id = context.factory_id`（与 `product_line_code` 联合），防跨工厂同产品线数据串读；`SameTypeProductKBSource` 跨产品线同 product_type 检索仍受 `factory_id` 收口（**仅 `context.factory_id` 内其他 PL 的同类型**，不跨工厂）。**唯一 skip/done(0) 规则**（R3-修复）：不再"返回 [] 据 summary 判 skipped"。既有源（FMEAGraphSource/SemanticSearchSource/HistoricalCAPASource/RuleEngineSource）的**结构性前置条件**（embedding None / linked_fmea None / pc None / D5 不适用）由编排器 `_stage_precondition` 集中处理（R3-修复），不要求既有源新增 should_skip。
...
- `enqueue_embedding(db, "capa_lesson", lesson.lesson_id, lesson.product_line_code, lesson.factory_id)` 入队，**embedding worker 必须把 `capa_lesson` 加入 `embedding_sync_worker.py:table_field_map`**（R12-修复：`"capa_lesson": ("capa_lessons_learned", "lesson_id", "product_line_code", None, ["lesson_text"])`，否则 worker `if entity_type not in table_field_map: continue` 跳过 → `document_embeddings` 无 lesson 行 → `LessonsLearnedSource` 长期无数据；P0 follow-up 已识别 worker 不认识 `agent_memory`，本 spec 同理需加 `capa_lesson`）。**d8 delete-and-rebuild 时同步清理孤儿 embedding（R8-修复）**：savepoint 内 `DELETE FROM document_embeddings WHERE entity_type='capa_lesson' AND entity_id IN (被删 d8 lesson_id)` + 取消对应 pending outbox 行，避免删改 lesson 文本残留 embedding。
...
2. **D7 闭环闸口（决策 18，转换前，R2+R5+R7+R8+R9-修复）**，按序：
   a. **canonical completeness check + FMEA 行锁（R9+R11-修复，生成前跑，不只在空集）**：`SELECT fmea_id FROM fmea_documents WHERE factory_id=capa.factory_id AND product_line_code=capa.product_line_code FOR UPDATE`（**锁 FMEA 行，防并发 FMEA 编辑在 hash check 后/commit 前改 graph 导致刚关闭的 CAPA stale，R11-修复 READ COMMITTED race**），与加载的 fmea_docs 数对比——**任何不一致（partial preload：加载部分 FMEA）→ fail-closed `ValueError("D7 推荐重算异常：FMEA 预加载不完整")` → 400**（R9-修复：不只在推荐为空时检查，partial preload 也拦截，避免只验证已加载子集而漏未加载 FMEA 的推荐）；`capa.fmea_ref_id` 非空但 fmea_docs 未含该 FMEA → fail-closed 400 "关联 FMEA 未加载"。FMEA 行锁持续到事务 commit（与 `SELECT capa FOR UPDATE` 一起序列化 FMEA 编辑 vs 闸口）。
...
4. **D7 lessons 抽取（决策 10/17，R2+R7+R12+R13-修复 fail-closed + 抽取先于状态 mutation + savepoint）**：**顺序：闸口通过 → 先抽 lessons → 再状态转换**（R12-修复：抽取先于 `capa.status=D8` mutation，失败时状态未动）。**`async with db.begin_nested()` savepoint 内**调 `_extract_lessons(db, capa, "d7")`（`text_override=None`，从 `capa.d7_prevention` 读）+ 写 `LESSON_EXTRACTED` audit（`correlation_id=uuid5(capa_id,"lesson_extract_d7")`）（**R13-修复：savepoint 包裹，`_extract_lessons` 内 insert/upsert + `enqueue_embedding` 任一步抛错 → savepoint rollback 撤销已 insert 的 lesson 行 + outbox 行，脏数据不留在当前事务**），异常 → `ValueError("D7 lessons 抽取失败，不可关闭，请重试")` → API 400，D8 **不推进**（状态未 mutation、无 TRANSITION audit、无 lesson 行，KB==CAPA 保证）。抽取成功后才 `capa.status = D8_CLOSURE` + 写 TRANSITION audit → commit。`_extract_lessons(db, capa, source_d_step, text_override=None)`：`text_override` 非 None 用它（d8 抽取传新 d8_closure，不 mutate capa 字段），None 从 capa 字段读（R13-修复接口匹配）。
...
D8_CLOSURE 是终态（无 D9 转换），`d8_closure` 文本在 D8 期间可编辑。`backend/app/services/capa_service.py:update_capa`（或 CAPA 字段保存路径）在检测到 `d8_closure` 字段变更且 `capa.status == D8_CLOSURE` 时，**R12-修复：传新 d8_closure 文本，不先 mutate `capa.d8_closure`**，调 `_extract_lessons(capa, source_d_step="d8", text_override=new_d8_closure)`，**delete-and-rebuild + savepoint + fail-closed（R3 stale + R4 atomic + R7 防并发/读路径副作用 + R12 抽取先于字段 mutation）**：
1. 开 savepoint（SQLAlchemy `async with db.begin_nested()`）。
2. `DELETE FROM capa_lessons_learned WHERE capa_id=X AND source_d_step='d8'`；**同步清理孤儿 embedding + claim-safe（R8+R9+R12-修复）**：先 **`UPDATE embedding_sync_outbox SET status='cancelled' WHERE entity_type='capa_lesson' AND entity_id IN (被删 d8 lesson_id) AND status='pending'`**（R12-修复：真实表名 `embedding_sync_outbox`，非 `embedding_outbox`；锁掉未认领的 pending 行，防 worker 认领后写 stale），再 `DELETE FROM document_embeddings WHERE entity_type='capa_lesson' AND entity_id IN (被删 d8 lesson_id)`；**已认领/in-flight 的 outbox 行由 worker 兜底**：embedding worker upsert `document_embeddings` 前**重查 `capa_lessons_learned` 行存在**（`SELECT 1 FROM capa_lessons_learned WHERE lesson_id=:id`），行已删 → 丢弃该 job 不写 embedding（R9-修复：防 in-flight worker 在 cleanup 后写 stale embedding）。删改 lesson 文本不残留 embedding。
...
5. **任一步异常 → ROLLBACK TO SAVEPOINT + 阻断保存**：`ValueError("D8 lessons 抽取失败，无法保存闭环总结，请重试")` → API 400，d8_closure **不保存**（**`capa.d8_closure` 未被 mutate**，R12-修复：抽取先于字段 mutation，失败时字段未动，外层事务无需回滚字段；KB==CAPA 保证，不残留 stale）。**抽取成功后**才 `capa.d8_closure = new_d8_closure` + release savepoint + 随外层事务 commit。
...
```
Spec A 的 `record_d7_action` / `auto_fill_d7` 在 insert/升级 `CapaD7NodeAction` 时调 `recommendation_fingerprint(...)`（从当前 D7 推荐取字段），存入 `recommendation_hash`。D7 闸口（决策 18）重算当前推荐时调**同一** `recommendation_fingerprint(...)` 生成 `current_hash`，要求动作 `recommendation_hash == current_hash`——FMEA 节点改名/推荐内容变 → hash 变 → 旧动作 stale → 闸口阻断。既有 Spec A 动作行（hash NULL）→ 视为 stale。**R13-修复：若 `record_d7_action`/`auto_fill_d7` 收到的 key 不在当前 D7 推荐集中（stale UI / 恶意/重复 key）→ 立即 `ValueError("D7 推荐 key 不存在于当前推荐集")` → API 400，不落 action**（防 stale/恶意 key 先写入，之后 gate 再失败）。**测试**：record + gate 用 byte-identical 输入 → hash 相等；改名任一 name 字段 → hash 变；key 不在当前推荐 → 400。
...
- `backend/tests/recommendation/test_orchestrator.py`：
  ...
  - **should_skip 强制 + callable + async + per-stage + 强制 lint（R4+R5+R6+R10+R11-修复）**：mock 某新源缺 should_skip / 非 callable（设成属性）/ 非 async → 该 stage `error`（不阻断整请求，其余 stage + 12 阶段响应照常，**不**在构造时 raise）；**`test_new_source_protocol_lint.py`（强制 pytest，R11-修复）：构造 `RecommendationOrchestrator` 调 `validate_all_new_sources()` 断言返回 `[]`——入 CI 门控，配置错误 deploy 前即红**；**`app.main` startup hook 调 `validate_all_new_sources()`，违规 → `logger.error`（非阻断启动，health check 暴露）**。
...
- `backend/tests/capa/test_capa_d7_gate.py`（R2-决策 18）：
  ...
  - **stale 处置（R10+R11-修复）**：CAPA 有 D7 推荐 key K（fm_name="虚焊"），用户 confirmed（动作存 `recommendation_hash=recommendation_fingerprint(...)`）；之后 FMEA 节点改名 fm_name="虚焊2" → 闸口重算当前 `recommendation_fingerprint` ≠ 动作 hash → key K 视为 stale/未处置 → 400 "已 stale（FMEA 变更）"，强制重新处置；既有 Spec A 动作（hash NULL）→ 视为 stale → 需重新处置；**record + gate 用同一 `recommendation_fingerprint` helper，byte-identical 输入 → hash 相等（R11-修复 delimiter/字段单源）**。
  - **并发 FMEA 编辑 race（R11-修复）**：闸口 `SELECT fmea_documents ... FOR UPDATE` 锁 FMEA 行 → 并发 FMEA 编辑阻塞到闸口事务 commit；断言并发 FMEA 编辑 + advance 时，advance 看到的 FMEA graph 与 hash check 一致（不 stale）。
...
- **should_skip 强制 + callable + async + per-stage + 强制 lint（R4+R5+R6+R10+R11-修复）**：6 新源 `should_skip` 必须存在、可调用、`iscoroutinefunction`（async）；**协议校验 per-stage 运行时**——`_exec_recall_stage` 调 `_check_source_protocol`，违规 → 该 stage `error`（不阻断构造/整请求）；**强制 lint（R11-修复）：`test_new_source_protocol_lint.py` 入 CI 门控（断言 `validate_all_new_sources()==[]`）+ `app.main` startup hook `logger.error` 暴露违规**——配置错误 deploy 前即红，不靠运行时降级发现；编排器 `await should_skip(context)`（async DB 查存在性，不误判 coroutine truthy）。
...
- 后端新增 pytest 全绿（含 R1~R10 各项 + R11 四项回归：canonical `recommendation_fingerprint` 单源 record/gate / D7 闸口 FMEA 行锁防并发编辑 race / LLM 全失败返回原 fused 不丢推荐 / 源协议强制 lint 入 CI + startup hook）；既有推荐/D7/capa 测试不退化；前端 vitest + `tsc --noEmit` + `npm run build` 绿；`make check` 绿。
