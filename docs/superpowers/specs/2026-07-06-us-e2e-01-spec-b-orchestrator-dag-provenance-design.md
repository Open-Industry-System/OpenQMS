...
4. **D7 lessons 抽取（决策 10/17，R2+R7+R12-修复 fail-closed + 抽取先于状态 mutation）**：**顺序：闸口通过 → 先抽 lessons → 再状态转换**（R12-修复：抽取先于 `capa.status=D8` mutation，失败时状态未动，事务回滚干净）。调 `_extract_lessons(capa, source_d_step="d7")`：**仅从 `d7_prevention` 切句**（d7_prevention 在 D7 已定稿；`d8_closure` 此时为空，**不在此抽取**——见下方 d8_closure 更新钩子），逐句构造 `CapaLessonLearned`（`lesson_id = uuid5(NAMESPACE_URL, f"capa_lesson:{capa_id}:d7:{normalized_text}")`），用 `pg_insert(...).on_conflict_do_update(index_elements=["lesson_id"], set_={category, tags, updated_at})` upsert，`enqueue_embedding(db, "capa_lesson", lesson_id, ...)` 入队。**抽取异常 → 阻断转换**：`ValueError("D7 lessons 抽取失败，不可关闭，请重试")` → API 400，D8 **不推进**（状态未 mutation、无 TRANSITION audit、无 lesson 行，KB==CAPA 保证，R7+R12-修复）。抽取成功后才 `capa.status = D8_CLOSURE` + 写 TRANSITION audit → commit。
...
D8_CLOSURE 是终态（无 D9 转换），`d8_closure` 文本在 D8 期间可编辑。`backend/app/services/capa_service.py:update_capa`（或 CAPA 字段保存路径）在检测到 `d8_closure` 字段变更且 `capa.status == D8_CLOSURE` 时，**R12-修复：传新 d8_closure 文本，不先 mutate `capa.d8_closure`**，调 `_extract_lessons(capa, source_d_step="d8", text=new_d8_closure)`，**delete-and-rebuild + savepoint + fail-closed（R3 stale + R4 atomic + R7 防并发/读路径副作用 + R12 抽取先于字段 mutation）**：
