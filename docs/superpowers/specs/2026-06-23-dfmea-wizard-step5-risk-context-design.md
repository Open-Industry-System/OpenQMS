...
- **验收标准（强化）**：「措施先于打分」是硬约束，非软目标——**逐行**：某 cause 的 PC 或 DC `name` 为空时，该行的 **O/D** 两个 `InputNumber` 置为 `disabled`（无法打分），并在行内提示「先补全措施」。S 因失效影响/mode 级共享不禁用（详见 §2）。完成/finish 门禁（§5）作为第二道兜底。即：打分入口本身被锁，而非「能打分但完成后才报错」。
...
- **加载时序（避免 lock_version 竞态 + 数据完整性）**：`useWizardSave` 的 `lockVersionRef.current` 默认 `0`（useWizardSave.ts:17），若归一化在 `setLockVersion(doc.lock_version)`（DFMEAWizardPage.tsx:84）之前入队保存，会以 `lock_version: 0` 发请求 → 409。因此归一化**不得**走 `updateGraphData`。改为：在 `getFMEA().then(...)` 内，**先** `setLockVersion(doc.lock_version)`，**再**对 loaded nodes/edges 跑 `ensureCauseControls`，把归一化后的数组直接 `setNodes`/`setEdges`；仅当 `changed` 时，用 `immediateSave(normalizedGraphData, title, normalizedHash)` 触发一次后台保存修复后端图。
  - **`lastSavedHashRef` 处理（完整性）**：**不要**提前把 `lastSavedHashRef.current` 写为归一化后的 hash。`immediateSave` 的第三参 `dataHash` 会被 hook 在**保存成功后**才写入 `lastSavedHashRef`（useWizardSave.ts:84）。这样：保存成功 → hash 自然标 clean；保存失败 → `lastSavedHashRef` 仍为加载时的旧 hash（归一化后的状态与之不等）→ `beforeunload` 会提示未保存，后端图虽未修复但用户被警告，不会静默丢失。归一化后的 `setNodes`/`setEdges` 仍立即生效，UI 不阻塞。
  - **加载基线 hash**：加载时 `lastSavedHashRef.current` 设为**归一化前**的 loaded hash（即后端当前状态），与 `computeHash(loadedNodes, loadedEdges, loadedScope)` 一致——保持现状（DFMEAWizardPage.tsx:86），不动。归一化若 `changed`，前端状态与该基线出现受控差异，由 `immediateSave` 成功后对齐。
  - `t` 不再传入 `ensureCauseControls`（见契约）。
...
**逐行打分门禁（兑现「措施先于打分」）**：每行计算 `pcName`/`dcName`（取首个 PC/DC 节点 name，trim）。当 `!pcName || !dcName` 时，该行 **O/D** 两个 `InputNumber` 设 `disabled`，并在 AP 列位置展示 `<Tag>` 文案「先补全措施」（`wizard.risk.controlsFirst`）。PC/DC 均非空时恢复正常可编辑。归一化（§0）保证节点存在，故此处只需判 name 非空。

> **S 的共享语义**：S 来自 `FailureEffect.severity`（mode 级，同 mode 多 cause 共享，`getRowSeverity` 取 max），不是 cause 私有值。因此 S 的输入入口仍按行渲染（与现状一致），但「禁用」语义只作用于 O/D：某行 PC/DC 未填时禁用该行 O/D，S 不禁用——因为同一 mode 下另一行 PC/DC 已填时用户可能已在该行编辑 S，S 值会被同 mode 所有行共享显示。若 mode 下所有 cause 的 PC/DC 均未填，S 也无意义，但 finish 门禁（§5）会挡住。文案上「先补全措施」Tag 仅提示该行 O/D 被锁。
...
逐行打分门禁已在 §2 UI 层落地（PC/DC 空则 O/D disabled；S 因 effect/mode 级共享不禁用）。此处 `step5MissingControl` 作为**第二道兜底**，阻塞 finish：某 cause 的 PC 或 DC `name` 为空（trim 后）即为 true。`step5Complete` 追加 `&& !step5MissingControl`；`warnings` 在 `rows.length > 0 && !step5Complete` 时仍 push `4`（第 5 步标红）。`canFinish`（DFMEAWizardPage.tsx:157-160）因依赖 `step5Complete` 自动阻塞。
