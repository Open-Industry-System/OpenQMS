...
| `unit` | String(20)? | 单位 |
| `supplier_id` | UUID → suppliers | 关联供应商 |
...
judged(rejected) 状态下可申请复检，不引入新状态，而是设置 `re_inspection=True` 后回到 `inspecting`：

```
judged(rejected) → re_inspection=True → inspecting → judged → closed
```

复检标记 `re_inspection=True`，状态回退到 `inspecting`。复检判定后结果覆盖原结果，原结果保留在 AuditLog 中。
...
创建检验单时自动调用 `aql_engine.calculate_aql_plan(lot_qty, aql_level, inspection_level)`，将结果写入 `code_letter`、`sample_qty`、`accept_number`、`reject_number`。
...
- 为 `iqc_inspections` 表新增字段（inspection_mode, material_id, template_id, code_letter, accept_number, reject_number, status, re_inspection, product_line_code, linked_scar_id, judged_by, judged_at）
