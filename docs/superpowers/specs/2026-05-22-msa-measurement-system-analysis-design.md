...
├── product_line_code VARCHAR(20)   -- 产品线编码，FK → product_lines.code
├── name (String)                   -- 量具名称
├── model (String)                  -- 型号规格
...
```sql
bias_studies
├── study_id / study_no / product_line_code VARCHAR(20) / title / gauge_id / characteristic_name
...
```sql
attribute_studies
├── study_id / study_no / product_line_code VARCHAR(20) / title / gauge_id (nullable)
...
> **校验规则**: When gauge_id is referenced from control_plan_items or SPC inspection_characteristics, the service layer must validate that the gauge's status is 'active' AND next_calibration_date > today(). If validation fails, raise ValueError('量具当前状态不可用或已超过校准有效期')。
...
POST /api/msa/{study_type}/{id}/import-spc-data
                                    从 SPC sample_batches 自动导入测量数据（仅 bias/stability 研究可用）
                                    Body: { spc_characteristic_id, date_range_start?, date_range_end? }
                                    返回: { imported_count, measurements: [...] }
```

> **数据导入流程**: `import-spc-data` 端点根据 `spc_characteristic_id` 查询 SPC `sample_batches` 表中符合条件的批次数据，将 `sample_value` / `sample_mean` / `sample_range` 自动映射为对应研究的测量记录（bias→一维 value 序列，stability→date+mean+range 时间序列）。导入前校验目标研究状态为 draft/ongoing，导入后测量数据可通过 `POST /measurements` 接口继续手工补全。
