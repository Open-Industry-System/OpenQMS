...
1. **客户档案**  
   共享客户主数据，不按产品线隔离。包含客户编号、名称、行业分类、联系人、PPM 目标、CSR 列表。
...
**索引**: `(product_line_code, status)`, `(customer_id, received_date)`, `(due_date)`, `(capa_ref_id)`, `(fmea_ref_id)`
...
**索引**: `(product_line_code, status)`, `(customer_id, received_date)`, `(complaint_id)`, `(responsibility)`
...
其中 `independent_rma` 只包含未关联客诉的 RMA，避免同一质量事件在客诉和退货中重复计数。由于本期没有发运数据模块，`shipment_qty` 采用前端看板筛选项或客户档案临时输入值。未提供发运数时，看板显示投诉数、退货数和影响数量，不显示 PPM 判定。
...
| 方法 | 路径 | 权限 | 说明 |
|------|------|------|------|
| GET | `/api/customers` | viewer+ | 列表，支持 `q`、`segment`、分页 |
...
| 方法 | 路径 | 权限 | 说明 |
|------|------|------|------|
| GET | `/api/customer-complaints` | viewer+ | 列表，支持 `product_line`、`customer_id`、`status`、`severity`、`overdue` |
...
| 方法 | 路径 | 权限 | 说明 |
|------|------|------|------|
| GET | `/api/rma-records` | viewer+ | 列表，支持 `product_line`、`customer_id`、`complaint_id`、`status`、`responsibility` |
...
- 客诉详情：基础信息、分析字段、CAPA/FMEA 关联、创建 8D、关闭。
- RMA 详情：退货信息、分析结果、责任判定、CAPA/FMEA 关联、关闭。
...
- `npm run build`
- 页面手动 smoke：客户质量菜单、客户列表、客诉创建、RMA 创建、看板加载、产品线过滤。
