...
- [x] 完成 客户审核管理 (审核日程 + 发现项追踪 + 客户确认 + 整改闭环)
- [x] 完成 批量导入/导出 Excel (通用 backend utils + frontend ImportExcelDialog，已覆盖供应商/SPC/IQC 模块)
- [x] 完成 SCAR 接入 scar_ref_id (供应商责任客诉/RMA 一键创建 SCAR + 双向关联)
- [x] 完成 0 公里 PPM 发运记录 (shipment_records + PPM fallback 链)
- [x] 完成 CSR/VOC → 控制计划同步 (客户特殊要求一键同步)
- [x] 完成 高级客户质量看板 (SPC CPK + 保修 + 满意度 + 审核摘要 + 产品线过滤)
